...
- 削除方針はテーブルごとに決める。日次ログ・streak と紐づくマスタ（`goals` / `habit_groups` / `habit_items`）は `is_active = false` の論理削除とし、過去ログを保持する。Wish List（`wish_categories` / `wish_items`）は保持すべき履歴が無いため物理削除（FK CASCADE）とする。詳細は [`api-design.md`](api-design.md) の各 `DELETE` を参照
...
| `is_active` | boolean | 論理削除用。削除時も `goal_logs` を保持する |
...
| `content` | text | 習慣本文 |
| `is_active` | boolean | 論理削除用。削除時も `habit_item_logs` を保持する |
| `sort_order` | integer | 表示順 |
| `created_at` | timestamp | 作成日時 |
| `updated_at` | timestamp | 更新日時 |
| `version` | integer | 楽観ロック用。初期値 1 |
